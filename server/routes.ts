...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(
    "/api/demo-videos/meeting-recording.mp4",
    (req: Request, res: Response) => {
      // نعيد توجيه الطلب إلى أحد مقاطع الفيد��و العامة
  ...
  app.get("/api/thumbnails/video-poster.jpg", (req: Request, res: Response) => {
    ...
    res.redirect(
      "https://img.freepik.com/free-vector/video-conference-remote-working-flat-illustration-screen-laptop-with-group-colleagues-people-conn_88138-548.jpg",
    );
  });
  ...
  if (process.env.NODE_ENV === "development") {
    app.get("/api/debug/users", async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "حدث خطأ أثناء استرجاع قائمة المستخدمين" });
      }
    });
  ...
  app.get("/api/talents/:id", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch football talent details" });
    }
  });
  ...
  app.get("/api/talent/stats", async (req, res) => {
    try {
      const talentId = req.query.talentId
        ? parseInt(req.query.talentId as string)
        : null;
  ...
  app.get("/api/coach/achievements", async (req, res) => {
    try {
      ...
      const achievements = coach.achievements
        ? coach.achievements.split(",").map((item) => item.trim())
        : [];
  ...
  app.get("/api/compare-stats", async (req, res) => {
    try {
      ...
      if (!idsParam) {
        return res
          .status(400)
          .json({ message: "Football talent IDs are required" });
      }
  ...
  app.get("/api/media/user/:id", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "حدث خطأ أثناء جلب ��لفات الوسائط" });
    }
  });
          ...
          const sampleFiles = [
            {
              ...
              url:
                talent.imageUrl ||
                "https://images.unsplash.com/photo-1571988840298-3b5301d5109b?ixlib=rb-1.2.1&auto=format&fit=crop&w=600&q=80",
            ...
            },
            {
              ...
              type: "video",
              url:
                talent.videoUrl || "https://www.example.com/sample-video.mp4",
    ...
    try {
      ...
      if (message.isFromUser) {
        const autoResponse = {
          ...
          message:
            "شكراً لتواصلك! سيقوم ��حد مستشاري النادي بالرد على استفسارك قريباً. هل هناك أي معلومات محددة ترغب في معرفتها عن خدماتنا؟",
          isFromUser: false,
        };
  ...
  app.get("/api/signing-video", async (req: Request, res: Response) => {
    try {
      const cachedDataPath = path.join(
        ...
        "public",
        "generated-images",
        "cached-video-data.json",
      );

      // التحقق من وجود ��يانات مخزنة مسبقاً لتجنب استدعاء OpenAI في كل مرة
      if (fs.existsSync(cachedDataPath)) {
        try {
          const cachedData = JSON.parse(
            fs.readFileSync(cachedDataPath, "utf8"),
          );

          // التحقق من اكتمال البيانات المخزنة
          if (
            cachedData.images &&
            cachedData.images.length >= 3 &&
            cachedData.captions &&
            cachedData.captions.length >= 3
          ) {
    ...
    ws.on("message", async (message) => {
      try {
        ...
        if (data.type === "join-session") {
          console.log(
            `User ${data.userId} joined video session ${data.sessionId}`,
          );
        ...
        if (data.type === "signal") {
          console.log(
            `Relaying signal from user ${data.userId} to user ${data.receiverId}`,
          );

          // Find the recipient connection
          const recipientConnection = connections.find(
            (conn) =>
              conn.userId === data.receiverId &&
              conn.sessionId === data.sessionId,
          );

          if (
            recipientConnection &&
            recipientConnection.ws.readyState === WebSocket.OPEN
          ) {
        ...
        if (data.type === "message") {
          ...
          setTimeout(async () => {
            const autoResponse = {
              ...
              doctorId: connection.doctorId,
              message:
                "شكراً لتواصلك! سيقوم أحد مستشاري النادي بالرد على استفسارك قريباً. هل هناك أي معلومات محددة ترغب في معرفتها عن خدماتنا؟",
              isFromUser: false,
            };
            ...
            connections.forEach((conn) => {
              if (
                (connection.userId && conn.userId === connection.userId) ||
                (connection.talentId &&
                  conn.talentId === connection.talentId) ||
                (connection.clubId && conn.clubId === connection.clubId) ||
                (connection.agentId && conn.agentId === connection.agentId) ||
                (connection.doctorId && conn.doctorId === connection.doctorId)
              ) {
    ...
    ws.on("close", () => {
      ...
      if (index !== -1) {
        ...
        if (disconnectedConn.sessionId) {
          console.log(
            `User ${disconnectedConn.userId} left video session ${disconnectedConn.sessionId}`,
          );

          // Notify other users in the same session
          connections.forEach((conn) => {
            if (
              conn.sessionId === disconnectedConn.sessionId &&
