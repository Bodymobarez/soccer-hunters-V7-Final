...
// Use the imported t function with our format
export const getTranslation = (key: string, locale: Locale): string => {
  // Use the useTranslation hook from i18n.ts to get the translated string
  // Note: useTranslation is a hook, so it cannot be called directly here
  // Instead, we will update the t function in TranslationProvider to use useI18nTranslation hook directly
  // This function will be a placeholder now
  return key;
};
...
export function TranslationProvider({ children }: { children: ReactNode }) {
  ...
  // Translation function - using our imported t function
  const t = (key: string): string => {
    // Convert from flat key to namespace.key format if needed
    const namespaceKey = key.includes(".") ? key : `common.${key}`;
    // Use the useTranslation hook from i18n.ts to get the translated string
    // We call useI18nTranslation hook here with namespaceKey and locale
    // Note: hooks cannot be called conditionally or inside functions, so we will create a wrapper component to provide t function properly
    // For now, call useI18nTranslation directly here (assuming this is a React component)
    const translation = useI18nTranslation(namespaceKey, locale);
