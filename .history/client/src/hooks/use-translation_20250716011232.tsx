...
export const getTranslation = (key: string, locale: Locale): string => {
  // Convert from flat key to namespace.key format
  const namespaceKey = key.includes(".") ? key : `common.${key}`;
  return t(namespaceKey, locale);
};
...
export function TranslationProvider({ children }: { children: ReactNode }) {
  ...
  const t = (key: string): string => {
    ...
    const namespaceKey = key.includes(".") ? key : `common.${key}`;
    const translation = getTranslation(namespaceKey, locale);
  ...
  return (
    <TranslationContext.Provider value={{ t: translateKey, locale, setLocale, dir }}>
      {children}
